component implements="IStrategy" singleton {
	...
	private numeric function getPercentage( required any percentage ) {
		return clamp(
			0,
			isNumeric( arguments.percentage ) ? arguments.percentage : 0,
			100
		);
	}

	private numeric function clamp(
		required numeric low,
		required numeric actual,
		required numeric high
	) {
	...
	private numeric function getNormalizedNumber(
		required string identifier,
		required string groupId,
		numeric normalizer = 100
	) {
	...
	private any function calculateHash( required array bytes ) {
		var hash = variables.murmur3.hash_x86_32(
			arguments.bytes,
			len( arguments.bytes ),
			0
		);
